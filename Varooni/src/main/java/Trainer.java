...
public class Trainer {
    ...
    public Trainer(String server, int port, String username, int n) {
        //super(server, port, username);
        name = username;
        grid = new int[n][n];
        db = DBMaker.fileDB(new File(username)).cacheHashTableEnable().cacheSize(5000000).make();
//        db2 = DBMaker.fileDB(new File("AI-2")).cacheHashTableEnable().cacheSize(5000000).make();
        seenStates = db.hashMapCreate("States").makeOrGet();
//        seenStates2 = db2.hashMapCreate("States").makeOrGet();
    }
...
//    }
    ...
    public void updateBoard(String opponentPosition){
        ...
        //if (opponentPosition.matches("[A-Za-z0-9-_]+ \\d+ \\d+")) {
...
//        db.compact();
    ...
    public static void main(String[] args) {
        ...
        Trainer client = new Trainer("localhost", port, name, 1000);
        //client.start();
    }
}